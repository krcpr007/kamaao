...
function View_product() {
    ...
    const [ rohitproductlist , setProductlist] =useState([]);
  function showlist(){
...
useEffect(() =>{
  ...
  function product_disable(id){
   axios.post('api/disable_product/'+id).then((res)=>{
 ...
 function product_enable(id){
  axios.post('api/enable_product/'+id).then((res)=>{
    if(res.data.status === 200){
      swal('success',res.data.message,'success');
    ...
    <div className='card'>
      <div className='card-body'>
          <div className="fun_btngroub">
            <Link to={`edit_product/${item.id}`} className='btn btn-info btn-sm'>Edit</Link> <span className='btn btn-danger btn-sm' onClick={()=>product_delete(item.id)}>Delete</span> {(item.status=='disable')?<span className='btn btn-success btn-sm' onClick={()=>product_enable(item.id)}>Enable</span>:<span className='btn btn-warning btn-sm' onClick={()=>product_disable(item.id)}>Disable</span>}
          </div>
        <div className='row'>
          ...
          <div className='col-4  '>
            {(()=>{
              if(item.company_logo=="Null"){
                ...
                return(
                  <img src={(item.product_img==null)?defaultSrc:`/company/`+item.product_img} className='cardImage  CardImageBox' />
                );
              }
            })()}
          </div>
          <div className='col-8'>
            <h3 className='First_p'>{item.product_name}</h3>

            <h4 className='trird_p'>RS {item.min_price} - {item.max_price}</h4>
          </div>
        </div>
        <div className='row mt-2'>
          {/* {/* <div className='col-3 cardButton'><a className='buttonText'>{item.total_openings} Total opening</a></div> */}
          <div className='col-5 cardButton'><a className='buttonText'>Discount {item.discount} % </a></div>
        </div>
      </div>
    </div>
  </div>);
            ...
            <Breadcrums name='View Product' />
                ...
                <div className='row'>
                    <div className='col-12 cardHeader'>
                        ...
                        <Link to='/admin/add_product'><AddBoxIcon className='PlusIcon' /></Link>
