...
function ApplicationRefferedTable() {
  ...
  function showApplication(){
    axios.get('api/view_application_refferd').then((res) => {
...
function diffDays(date1,date2){
  ...
  const data = application.map((response,index) => (
    
    {
      ...
      available_time:diffDays(new Date(current_date), new Date(response.end_time))
  ...
  <div>
    ...
    <div className="modal fade" id="exampleModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <Table
        ...
        rowClassName={(record, index) =>((record.available_time <=5  )? "bg-danger" : (record.available_time <= 15)?'bg-warning':'bg-success')}
      />
  </div>
  );
     }
...
export default ApplicationRefferedTable;